...
function FilterSelect({
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      >
        {options.map((o) => (
          <option key={o.value} value={o.value} className="bg-bg3 text-text">
            {o.label}
          </option>
        ))}
      </select>
...
export function FilterBar() {
  ...
  return (
    <div data-ui="filter-bar" className="sticky top-0 z-30 flex flex-wrap items-center gap-4 border-b border-border bg-bg2 px-6 py-2">
      <FilterSelect label="Division" value={division} options={DIVISIONS} onChange={(v) => setDivision(v as Division)} />
